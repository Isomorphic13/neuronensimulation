...
class NeuronalNetwork:
    ...
    def get_voltage_on_single_cell(self,x,y) -> np.ndarray:
        rows, cols = self.weights.shape
        voltages = np.empty((self.number_of_layers, self.neurons_in_layer), dtype = object)
        ...
        for j in range(cols): #going through a single layer i and saving voltages inside each neuron
            outer_current = np.zeros_like(self.calculating_neuron.time_array) #storage for the outer current applied to a single neuron
            for k in range(len(prev_voltages)): #calculating the outer current
                 outer_current = outer_current + self.weights[x,k] * prev_voltages[k]
            total_current = outer_current + self._get_eigen_current(row = x, column = j) #total current with consideration of the input to the cell
            voltage_x_j = self.calculating_neuron.get_voltage(total_current) #calculating the voltage inside the cell
            voltages[x,j] = voltage_x_j #saving the voltage of the cell
            prev_voltages.append(voltage_x_j)
            if j == y:
                return voltages[x,j]

        return None

    def get_current_on_single_cell(self,x,y) -> np.ndarray:
        rows, cols = self.weights.shape
        voltages = np.empty((self.number_of_layers, self.neurons_in_layer), dtype = object)
    ...
    def give_answer_predefined_weights(self) -> bool:
        ...
        #checking if any peak falls within this region and if there are two peaks in the whole time interval
        peaks_in_region = peak_times[(peak_times >= region_start) & (peak_times <= region_end)]
        has_peak = len(peaks_in_region) > 0
        if has_peak and peaks_number == 3:
            return True
